...
6. 함수가 실행될경우 static a변수는 미리 사용됬던 a의 값을 그대로 사용한다.
...
두개의 주소는 다르다..
라는 것은! 이런식으로 contants::pi를 많이 사용하게 되면 그만큼 낭비하게 된다는 것이다.
어떻게 하면 이러한 낭비를 방지 할수있을까.
일단 cpp파일을 하나 더 만든다.
그곳에 namespace constatns의 **정의를한다.**
그리고 헤더에는 선언만 해준다.